...
unsigned long lastEmailTime = 0; // Track the last email send time
const unsigned long emailInterval = 30000; // 30 seconds interval
...
void sendEmail(float heartRate, float spo2) {
    ...
    message.addRecipient("Recipient Name", "30942csiot@gmail.com");
...
void loop() {
    ...
    if (millis() - lastEmailTime >= emailInterval) {
        lastEmailTime = millis(); // Update the last email time
